# Sullivan プロジェクト Codex 指示

このファイルは Codex 向けのガイドです。`CLAUDE.md` と同等の内容を Codex の指示ファイル仕様に読み替えています。
Codex ではルートの `AGENTS.md` がリポジトリ全体の指示として扱われます。
...
- PR を `main` にマージしたら、**必ず `/sync-clean` を実行する**
  - `main` / `dev` を `origin` と同期し、それ以外のローカルブランチを削除するため
  - マージ直後にローカルが古い状態のまま新しい作業を始めないようにする
