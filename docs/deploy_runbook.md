...
### 2.1 Supabase
- `NEXT_PUBLIC_SUPABASE_URL`（クライアント配信のため公開、GitHub Environment Secrets）
- `NEXT_PUBLIC_SUPABASE_ANON_KEY`（同上）
- `SUPABASE_SERVICE_ROLE_KEY`（**Secret Manager: `supabase-service-role-key`** に保存）
...
- `SUPABASE_SERVICE_ROLE_KEY` は Cloud Run runtime に SM 経由で注入する。GitHub Secrets には保持しない。
...
- `DATABASE_URL`: トランザクション用（SupabaseのTransaction Mode推奨）。Cloud Run runtime は SM `database-url` 経由。
- `DIRECT_URL`: マイグレーション用（SupabaseのSession Mode推奨）。Cloud Run runtime / worker は SM `direct-url` 経由。
- CI の `prisma migrate deploy` 実行は GitHub Environment Secret の `DATABASE_URL` / `DIRECT_URL` を使用 (将来的に WIF + SM へ寄せる予定)。
...
### 2.5 Gemini API
- `GEMINI_API_KEY`（**Secret Manager: `gemini-api-key`** に保存。GitHub Secrets には保持しない）
- `GEMINI_MODEL`（任意、モデル名なので非機密）
...
- `INTERNAL_API_SECRET`（**Secret Manager: `internal-api-secret`** に保存。GitHub Secrets には保持しない）
...
- `DRIVE_WEBHOOK_TOKEN`（**Secret Manager: `drive-webhook-token`** に保存。GitHub Secrets には保持しない）
...
### 3.3 Secret Manager（runtime シークレット）

Cloud Run の runtime に注入される secret は Secret Manager で一元管理する。Cloud Run は
`--update-secrets NAME=secret-name:latest` で各 secret を環境変数として注入する。

管理対象:

| Secret 名 (SM) | 環境変数 | 用途 |
|---|---|---|
| `internal-api-secret` | `INTERNAL_API_SECRET` | 内部 API 保護 (Drive watch renew / min-instances scheduler 等) |
| `drive-webhook-token` | `DRIVE_WEBHOOK_TOKEN` | Drive Push Notification 検証 |
| `gemini-api-key` | `GEMINI_API_KEY` | Gemini API 呼び出し |
| `supabase-service-role-key` | `SUPABASE_SERVICE_ROLE_KEY` | Supabase admin client |
| `database-url` | `DATABASE_URL` | Cloud Run runtime の DB 接続 (transaction pooler) |
| `direct-url` | `DIRECT_URL` | Prisma migrate 用 (session pooler) |

セキュリティ運用:
- リポジトリにシークレットの実値をコミットしない。`openssl rand -hex 32` 等でローカル生成してから直接 SM に書き込む。
- GitHub Environment Secrets には runtime シークレットを保持しない。CI が必要とする場合は WIF 経由で SM から取得する (例: workflow の Drive watch 再作成 step)。
...
# 2) Cloud Run 実行SA に Secret 参照権限を付与
