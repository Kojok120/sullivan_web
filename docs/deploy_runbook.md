...
### 2.8 アプリURL/内部シークレット
- `APP_URL`（WebサービスのURL）
- `GRADING_WORKER_URL`（WorkerのURL、Webサービスにのみ設定）
...
```bash
gcloud run deploy sullivan-grading-worker \
  --project "<PROJECT_ID>" \
  --service-account "<RUNTIME_SA_EMAIL>" \
  --source . \
  --region asia-northeast1 \
...
補足:
- WorkerはQStash署名検証が有効なため、**署名鍵が必須**です。
- Workerは `GRADING_WORKER_URL` を参照しないため設定不要。
